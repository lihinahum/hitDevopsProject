...
public class LoadTestPublic extends Simulation {
  private HttpProtocolBuilder httpProtocol = http
    .baseUrl("https://rjvedo-ip-5-29-14-185.tunnelmole.net")
    .inferHtmlResources()
  ...
  private ScenarioBuilder scn = scenario("TaskManagerLoadTest")  // Changed scenario name
    .exec(
      http("request_0")
  ...
  {
    // 5-minute Load test with the classic trapezoid pattern
    setUp(
      scn.injectOpen(
        rampUsersPerSec(0).to(300).during(Duration.ofMinutes(1)),
        constantUsersPerSec(300).during(Duration.ofMinutes(3)),
        rampUsersPerSec(300).to(0).during(Duration.ofMinutes(1))
      )
